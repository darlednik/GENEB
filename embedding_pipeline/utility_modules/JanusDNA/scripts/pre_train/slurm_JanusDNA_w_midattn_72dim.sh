#!/bin/bash
####SBATCH --job-name=janus_w_midattn      # Specify job name
...
D_MODEL=72
FLEX_ATTN_MODEL=128 # should be multiple of 2, and 64 is the minimum.
INTER_FFN_MODEL=288 # 4x is the best
...
WANDB_NAME="janusdna_len-${SEQLEN_DIS}_d_model-${D_MODEL}_inter_dim-${INTER_FFN_MODEL}_n_layer-${N_LAYER}_lr-${LR}_step-${GRADIENT_UPDATE_NUM}_moeloss-${OUTPUT_ROUTER_LOGITS}_1head_midattn"
...
srun python -m train \
  ...
  model.config.attn_layer_offset=4 \
  optimizer.lr="${LR}" \
